...
*/
        $data = $this->model->select('period_id','earnings','deductions','retro_pay','biometric_id','canteen','remarks','canteen_bpn','canteen_bps','canteen_agg','remarks2','cash_advance','office_account','retro_pay_nbp')
                    ->from('unposted_weekly_compensation')
                    ->where('period_id','=',$period_id);

        $employees = $this->model->select(DB::raw("employee_names_vw.biometric_id,employee_names_vw.employee_name,$period_id as period_id,ifnull(d.earnings,0.00) earnings,ifnull(d.deductions,0.00) deductions,ifnull(d.retro_pay,0.00) retro_pay,ifnull(d.canteen,0.00) canteen,d.remarks,d.remarks2,ifnull(d.canteen_bpn,0.00) canteen_bpn,ifnull(d.canteen_bps,0.00) canteen_bps,ifnull(d.canteen_agg,0.00) as canteen_agg,ifnull(d.cash_advance,0.00) as cash_advance,ifnull(d.office_account,0.00) as office_account,ifnull(d.retro_pay_nbp,0.00) as retro_pay_nbp"))
