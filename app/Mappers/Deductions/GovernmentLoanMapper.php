...
class GovernmentLoanMapper extends AbstractMapper {

	protected $modelClassName = 'App\Models\Deductions\GovernmentLoans';
    protected $rules = [
    ...
    public function list($biometric_id,$filter)
    {
        ...
        if($biometric_id!=0){
            $result = $result->where('deduction_gov_loans.biometric_id',$biometric_id);
        }
    ...
    {
        $result = DB::table('posted_loans')
                ->join('payroll_period','posted_loans.period_id','=','payroll_period.id')
